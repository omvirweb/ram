...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#add_lineitem').on('click', function() {
            ...
            line_item_des = line_item_des.replace(/["']/g, "");
    ...
    function display_lineitem_html(lineitem_objectdata, is_use_db_round_off_amount) {
        ...
        $.each(lineitem_objectdata, function(index, value) {
            ...
            if (value.discount_type == 1) {
                value_discount_type = 'Pct';
            }
            if (value.discount_type == 2) {
                value_discount_type = 'Amt';
            }
            ...
            var row_html = '<tr class="lineitem_index_' + index + '"><td class="fix_fcolumn" style="border-color:#2b3984;">' +
                ...
                <?php if (isset($invoice_line_item_fields) && in_array('item_group', $invoice_line_item_fields)) { ?> '<td style="border-color:#2b3984;">' + value_item_group_name + '</td>' +
                <?php } ?>
            <?php if (isset($invoice_line_item_fields) && in_array('category', $invoice_line_item_fields)) { ?>
                ...
                <?php if (isset($invoice_line_item_fields) && in_array('sub_category', $invoice_line_item_fields)) { ?>
                    ...
                    <?php } ?> '<td style="border-color:#2b3984;">' + item_name + '</td>' +
        ...
        $('tfoot span.pure_amount_total').html(pure_amount_total);
        $('#pure_amount_total').val(pure_amount_total);
        var discount_total = parseFloat(pure_amount_total) - parseFloat(discounted_price_total)

        $('tfoot span.discount_total').html('');
        $('#discount_total').val(discount_total);
        $('tfoot span.discounted_price_total').html(parseF(discounted_price_total));
        $('#discounted_price_total').val(discounted_price_total);

        $('tfoot span.cgst_amount_total').html(parseF(cgst_amount_total));
        $('#cgst_amount_total').val(parseF(cgst_amount_total));
        $('tfoot span.sgst_amount_total').html(parseF(sgst_amount_total));
        $('#sgst_amount_total').val(parseF(sgst_amount_total));
        $('tfoot span.igst_amount_total').html(parseF(igst_amount_total));
        $('#igst_amount_total').val(parseF(igst_amount_total));

        $('tfoot span.other_charges_total').html(other_charges_total);
        $('#other_charges_total').val(other_charges_total);
        $('tfoot span.amount_total').html(parseF(amount_total));
        $('#amount_total').val(parseF(amount_total));
