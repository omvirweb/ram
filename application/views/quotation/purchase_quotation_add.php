...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Purchase Quotation <?php if(isset($quotation_id) && !empty($quotation_id)){ echo 'Edit'; }else{ echo 'Add'; }  ?>
            <?php if($this->applib->have_access_role(MODULE_ORDER_TYPE_2_ID,"view")) { ?>
            <a href="<?=base_url('quotation/purchase_quotation_list');?>" class="btn btn-primary pull-right">Quotation List</a>
            <?php } ?>
        </h1>
    </section>
    <!-- Main content -->
    <section class="content">
        ...
        <div class="row">
            <div class="col-md-12">
                <form id="form_purchase_quotation" class="" action="" enctype="multipart/form-data" data-parsley-trigger="keyup" data-parsley-validate autocomplete="off">
                    <?php if(isset($quotation_id) && !empty($quotation_id)){ ?>
                    <input type="hidden" id="quotation_id" name="quotation_id" value="<?=$quotation_id;?>">
                    <?php } ?>
                    <input type="hidden" id="quotation_type" name="quotation_type" value="2">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#add_lineitem').on('click', function() {
            ...
            display_lineitem_html(lineitem_objectdata);
            $('#lineitem_id').val('');
            $('#form_purchase_quotation').find('input[name^="line_items_index"], input[name^="line_items_data"], select[name^="line_items_data"], textarea[name^="line_items_data"]').val('');
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on('submit', '#form_purchase_quotation', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (json['success'] == 'Added'){
                        window.location.href = "<?php echo base_url('quotation/purchase_quotation_add'); ?>";
                    }
                    if (json['success'] == 'Updated'){
                        window.location.href = "<?php echo base_url('quotation/purchase_quotation_list'); ?>";
                    }
                    $('.overlay').hide();
                    return false;
                },
