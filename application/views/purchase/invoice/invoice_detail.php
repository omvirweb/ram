<?php if(isset($purchase_invoice_id) && !empty($purchase_invoice_id)){ } else { ?>
	<script type="text/javascript">
		$(document).ready(function(){ window.location.href = '<?=base_url('purchase/invoice_list');?>'; });
	</script>
<?php } ?>
...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>
			...
			<a href="<?=base_url('purchase/invoice');?>" class="btn btn-primary pull-right" style="margin-right: 10px;" >Add New</a>
		</h1>
	</section>
	<!-- Main content -->
				...
				<div class="box box-primary">
					<div class="box-header with-border">
						<h3 class="box-title form_title">
							Detail &nbsp;
							<form id="edit_<?=$purchase_invoice_id; ?>" method="post" action="<?=base_url('purchase/invoice');?>" class="pull-right" style="width: 30px; display: initial;" >
								<input type="hidden" name="purchase_invoice_id" id="purchase_invoice_id" value="<?=$purchase_invoice_id; ?>" />
								<a class="edit_button btn-primary btn-xs" href="javascript:{}" onclick="document.getElementById('edit_<?=$purchase_invoice_id; ?>').submit();" title="Edit Invoice"><i class="fa fa-edit"></i></a>
							</form> 
						</h3>
					</div>
					<!-- /.box-header -->
					<div class="box-body">
						<div class="table-responsive">
							<table class="table">
								<tbody>
									<tr>
										<th style="width:25%">Purchase Invoice No.:</th>
										<td><?=isset($purchase_invoice_no) ? $purchase_invoice_no : '' ?></td>
										<th style="width:25%">Purchase Invoice Date:</th>
										<td><?=isset($purchase_invoice_date) ? $purchase_invoice_date : '' ?></td>
									</tr>
									<tr>
										<th>Supplier:</th>
										<td><?=isset($supplier) ? $supplier : '' ?></td>
										<th>Purchase Invoice Desc:</th>
										<td><?=isset($purchase_invoice_desc) ? $purchase_invoice_desc : '' ?></td>
									</tr>
									<tr>
										<th>Bill No:</th>
										<td><?=isset($bill_no) ? $bill_no : '' ?></td>
									</tr>
								</tbody>
							</table>
						</div>
							...
							<table class="table">
								...
								<tbody>
									...
									?>
										<tr>
											<td><?php echo $inc; ?></td>
											<td><?php echo $lineitem->purchase_order_no; ?></td>
											<td><?php echo $lineitem->purchase_challan_no; ?></td>
