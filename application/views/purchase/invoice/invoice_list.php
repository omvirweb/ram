...
<div class="content-wrapper">
            ...
            <?php
                if($invoice_type == 1) {
                    ...
                    if($this->applib->have_access_role(MODULE_ORDER_ID,"add")) {
                        echo '<a href="'.base_url('transaction/sales_purchase_transaction/order').'" class="btn btn-primary pull-right">Add New</a>';
                    }
                } elseif($invoice_type == 2) {
                    echo 'Purchase Invoice List';
                    if($this->applib->have_access_role(MODULE_ORDER_ID,"add")) {
                        echo '<a href="'.base_url('transaction/sales_purchase_transaction/purchase').'" class="btn btn-primary pull-right">Add New</a>';
                    }
    ...
    <input type="hidden" name="invoice_type" id="invoice_type" value="<?php echo isset($invoice_type) ? $invoice_type : ''; ?>">
    <input type="hidden" name="type" id="type" value="<?php echo isset($invoice_type) && $invoice_type == 1 ? 'Order' : 'Purchase'; ?>">
    <!-- Main content -->
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        table = $('.product-table').DataTable({
            ...
            "aaSorting": [[1, 'desc']],
            "ajax": {
                "url": "<?php echo base_url('purchase/invoice_datatable') ?>",
                "type": "POST",
                "data": function (d) {
