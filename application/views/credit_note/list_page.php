...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>
			...
			<a href="<?=base_url('credit_note/add');?>" class="btn btn-primary pull-right">Add New</a>
			<?php } ?>			
		</h1>
						...
						<table class="table table-striped table-bordered product-table" id="product-table">
							<thead>
								<tr>
									...
									<th width="100px">Credit Note No</th>
									<th>Account</th>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		table = $('.product-table').DataTable({
            dom: 'Bfrtip',
            buttons: [
                $.extend( true, {}, buttonCommon, { extend: 'copy', title: function () { return ('Credit Note (Sales Return) For '+client+ '( From Date : ' + $('#datepicker1').val() + ' To Date : ' + $('#datepicker2').val() + ' )')}, action: newExportAction } ),
                $.extend( true, {}, buttonCommon, { extend: 'csvHtml5', customize: function (csv) {
                                            return 'Credit Note (Sales Return) For '+client+ '( From Date : ' + $('#datepicker1').val() + ' To Date : ' + $('#datepicker2').val() + ' )\n\n'+  csv;
                                        }, action: newExportAction } ),
                $.extend( true, {}, buttonCommon, { extend: 'pdf', orientation: 'landscape', pageSize: 'LEGAL', title: function () { return ('Credit Note (Sales Return) For '+client+ '( From Date : ' + $('#datepicker1').val() + ' To Date : ' + $('#datepicker2').val() + ' )')}, action: newExportAction,
                ...
                $.extend( true, {}, buttonCommon, { extend: 'excelHtml5', title: function () { return ('Credit Note (Sales Return) For '+client+ '( From Date : ' + $('#datepicker1').val() + ' To Date : ' + $('#datepicker2').val() + ' )')}, action: newExportAction, 
                ...
                $.extend( true, {}, buttonCommon, { extend: 'print',  title: function () { return ('Credit Note (Sales Return) For '+client+ '( From Date : ' + $('#datepicker1').val() + ' TO Date : ' + $('#datepicker2').val() + ' )')},
			...
			"aaSorting": [[1, 'desc']],
			"ajax": {
				"url": "<?php echo base_url('credit_note/credit_note_datatable')?>",
                "type": "POST",
                "data": function(d){
		...
		$(document).on("click",".delete_button",function(){
			var value = confirm('Are you sure delete this Credit Note?');
			var tr = $(this).closest("tr");
			if(value){
				$.ajax({
					url: $(this).data('href'),
					type: "POST",
					data: 'id_name=credit_note_id&table_name=credit_note',
					success: function(data){
						table.draw();
						show_notify('Credit Note Deleted Successfully!',true);
					}
				});
			}
		});
