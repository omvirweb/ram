...
class Sales extends CI_Controller
{
	...
	function invoice(){
        ...
        $data = array();
        $data['module_id'] = MODULE_SALES_INVOICE_ID;
        $data['module_title'] = "Sales Invoice";
	...
	function save_sales_invoice(){
        ...
        $post_data['sales_invoice_date'] = $post_data['module_date'];
	...
	function invoice_datatable(){
		...
		$config['select'] = 'pd.transaction_id,si.invoice_type, si.sales_invoice_id, si.sales_invoice_no, si.sales_invoice_date, si.amount_total, si.data_lock_unlock, a.account_name';
		...
		foreach ($list as $invoice) {
			...
			if($invoice->data_lock_unlock == 0){
				if($isEdit) {
					if($this->is_single_line_item == 1) {
						...
						$action .= '<form id="edit_' . $invoice->sales_invoice_id . '" method="post" action="' . base_url() . 'sales/invoice" style="width: 25px; display: initial;" >
	                            <input type="hidden" name="sales_invoice_id" id="sales_invoice_id" value="' . $invoice->sales_invoice_id . '">
	                            <a class="edit_button btn-primary btn-xs" href="javascript:{}" onclick="document.getElementById(\'edit_' . $invoice->sales_invoice_id . '\').submit();" title="Edit Invoice"><i class="fa fa-edit"></i></a>
	                        </form> ';
					}
    		...
    		$row[] = $action;
                ...
                $sales_invoice_no = $this->applib->format_invoice_number($invoice->sales_invoice_no, $invoice->sales_invoice_date);
	...
	function invoice_print_pdf($sales_invoice_id){
		if(!empty($sales_invoice_id) && isset($sales_invoice_id)){
			...
			if($result->amount_total < 0){
				...
				$amount_total_word = $this->numbertowords->convert_number($result->amount_total);
                        ...
                        $this->load->library('applib');
                        $sales_invoice_no = $this->applib->format_invoice_number($result->sales_invoice_no, $result->sales_invoice_date);
			$data = array(
			...
			foreach($sales_invoice_lineitems as $sales_invoice_lineitem){
				...
				$sales_invoice_lineitem->hsn_code = $this->crud->get_id_by_val('item', 'hsn_code', 'item_id', $sales_invoice_lineitem->item_id);
...
//		echo '<pre>'; print_r($html); exit;
    ...
    function invoice_print_new_pdf($sales_invoice_id){
		if(!empty($sales_invoice_id) && isset($sales_invoice_id)){
			$result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
			$user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
...
//            echo '<pre>'; print_r($lineitem_arr); exit;
    ...
    function format_2_invoice_print($sales_invoice_id){
		if(!empty($sales_invoice_id) && isset($sales_invoice_id)){
			$result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
			$user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
...
//		echo '<pre>'; print_r($data); exit;
    ...
    function format_3_invoice_print($sales_invoice_id){
		if(!empty($sales_invoice_id) && isset($sales_invoice_id)){
			$result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
			$user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
			$account_detail = $this->crud->get_data_row_by_id('account', 'account_id', $result->account_id);
