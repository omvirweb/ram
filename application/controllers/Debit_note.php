...
class Debit_note extends CI_Controller
{
	...
	//~ Debit Note
	function add(){
		redirect('transaction/sales_purchase_transaction/debit_note');
		exit();
		...
		if(isset($_POST['debit_note_id'])){
			if($this->applib->have_access_role(MODULE_DEBIT_NOTE_ID,"edit")) {
				...
				$data['debit_note_data'] = $debit_note_data[0];
				$lineitems = '';
				...
				foreach($debit_note_lineitems as $debit_note_lineitem){
					//$debit_note_lineitem->pure_amount = $debit_note_lineitem->item_qty * $debit_note_lineitem->price;
	...
	function save_debit_note(){
		...
		$debit_note_data['account_id'] = $post_data['account_id'];
		$debit_note_data['debit_note_date'] = date('Y-m-d', strtotime($post_data['debit_note_date']));
		$debit_note_data['bill_no'] = $post_data['bill_no'];
		$debit_note_data['invoice_date'] = date('Y-m-d', strtotime($post_data['invoice_date']));
		$debit_note_data['debit_note_desc'] = $post_data['debit_note_desc'];
		$debit_note_data['qty_total'] = $post_data['qty_total'];
		$debit_note_data['pure_amount_total'] = $post_data['pure_amount_total'];
		$debit_note_data['discount_total'] = $post_data['discount_total'];
		$debit_note_data['cgst_amount_total'] = $post_data['cgst_amount_total'];
		$debit_note_data['sgst_amount_total'] = $post_data['sgst_amount_total'];
		$debit_note_data['igst_amount_total'] = $post_data['igst_amount_total'];
		$debit_note_data['amount_total'] = $post_data['amount_total'];
        ...
        if(!empty($dateranges)){
            foreach($dateranges as $daterange){
                ...
                $debit_note_date = date('d-m-Y', strtotime($post_data['debit_note_date']));
                if(strtotime($debit_note_date) >= strtotime($from) && strtotime($debit_note_date) <= strtotime($to)){
        ...
        if(isset($post_data['debit_note_id']) && !empty($post_data['debit_note_id'])){
			$debit_note_data['updated_at'] = $this->now_time;
			$debit_note_data['updated_by'] = $this->logged_in_id;
			$debit_note_data['user_updated_by'] = $this->session->userdata()['login_user_id'];
			$where_array['debit_note_id'] = $post_data['debit_note_id'];
			$result = $this->crud->update('debit_note', $debit_note_data, $where_array);
			if($result){
				$return['success'] = "Updated";
				$this->session->set_flashdata('success',true);
				$this->session->set_flashdata('message','Debit Note Updated Successfully');
				$parent_id = $post_data['debit_note_id'];
				if(isset($post_data['deleted_lineitem_id']) && !empty($post_data['deleted_lineitem_id'])){
                    ...
                    if(!empty($lineitems_res)) {
                        foreach ($lineitems_res as $key => $lineitem) {
                            $this->crud->update_item_current_stock_qty($lineitem->item_id,$lineitem->parent_id,'debit_note',$lineitem->item_qty,'delete');       
                        }
                    }
				...
				foreach($line_items_data[0] as $lineitem){
					...
					if(isset($lineitem->id) && !empty($lineitem->id)){
						...
						$this->crud->update_item_current_stock_qty($lineitem->item_id,$parent_id,'debit_note',$lineitem->item_qty,'update');
						...
						$this->crud->update_item_current_stock_qty($lineitem->item_id,$parent_id,'debit_note',$lineitem->item_qty,'add');
			...
			$debit_note_data['created_at'] = $this->now_time;
			$debit_note_data['created_by'] = $this->logged_in_id;
			$debit_note_data['user_created_by'] = $this->session->userdata()['login_user_id'];
			$debit_note_data['updated_at'] = $this->now_time;
			$debit_note_data['updated_by'] = $this->logged_in_id;
			$debit_note_data['user_updated_by'] = $this->session->userdata()['login_user_id'];
			
			$debit_note_no = $this->crud->get_max_number('debit_note', 'debit_note_no', $this->logged_in_id);
			$debit_note_data['debit_note_no'] = $debit_note_no->debit_note_no + 1;
			
			$result = $this->crud->insert('debit_note',$debit_note_data);
			if($result){
	...
	function list_page(){
		if($this->applib->have_access_role(MODULE_DEBIT_NOTE_ID,"view")) {
        	$data = array();
			set_page('debit_note/list_page', $data);
        } else {
        	$this->session->set_flashdata('success', false);
            $this->session->set_flashdata('message', 'You have not permission to access this page.');
        	redirect('/');
	...
	function debit_note_datatable(){
		...
		$config['table'] = 'debit_note dn';
		$config['select'] = 'dn.debit_note_id, dn.debit_note_no, dn.debit_note_date, dn.amount_total, dn.data_lock_unlock, a.account_name';
		$config['column_order'] = array(null, 'dn.debit_note_no', 'a.account_name', 'dn.debit_note_date', 'dn.amount_total');
		$config['column_search'] = array('dn.debit_note_no', 'a.account_name', 'DATE_FORMAT(dn.debit_note_date,"%d-%m-%Y")', 'dn.amount_total');
		$config['wheres'][] = array('column_name' => 'dn.created_by', 'column_value' => $this->logged_in_id);
        if (!empty($account_id)) {
            $config['wheres'][] = array('column_name' => 'dn.account_id', 'column_value' => $account_id);
        }
		...
		foreach ($list as $order) {
			...
			if($order->data_lock_unlock == 0){
				if($isEdit) {
					if($this->is_single_line_item == 1) {
						$action .= '<form id="edit_' . $order->debit_note_id . '" method="post" action="' . base_url() . 'transaction/sales_purchase_transaction/debit_note" style="width: 25px; display: initial;" >
	                            <input type="hidden" name="debit_note_id" id="debit_note_id" value="' . $order->debit_note_id . '">
	                            <a class="edit_button btn-primary btn-xs" href="javascript:{}" onclick="document.getElementById(\'edit_' . $order->debit_note_id . '\').submit();" title="Edit Debit Note"><i class="fa fa-edit"></i></a>
	                        </form> ';
					} else {
						$action .= '<form id="edit_' . $order->debit_note_id . '" method="post" action="' . base_url() . 'transaction/sales_purchase_transaction/debit_note" style="width: 25px; display: initial;" >
	                            <input type="hidden" name="debit_note_id" id="debit_note_id" value="' . $order->debit_note_id . '">
				...
				if($isDelete) {
					$action .= ' &nbsp; <a href="javascript:void(0);" class="delete_button btn-danger btn-xs" data-href="' . base_url('debit_note/debit_note_delete/' . $order->debit_note_id) . '"><i class="fa fa-trash"></i></a>';
				}
				
				$action_detail = ' &nbsp; <form id="detail_' . $order->debit_note_id . '" method="post" action="' . base_url() . 'debit_note/debit_note_detail" style="width: 25px; display: initial;" >
                            <input type="hidden" name="debit_note_id" id="debit_note_id" value="' . $order->debit_note_id . '">
                            <a class="detail_button btn-info btn-xs" href="javascript:{}" onclick="document.getElementById(\'detail_' . $order->debit_note_id . '\').submit();" title="Debit Note Detail"><i class="fa fa-eye"></i></a>
                        </form>';
			}
			$row[] = $action;
			$row[] = $order->debit_note_no;
			$row[] = $order->account_name;
			$row[] = date('d-m-Y', strtotime($order->debit_note_date));
	...
	function debit_note_delete($debit_note_id){
		$this->crud->kasar_entry(0,$debit_note_id,'debit_note',0,'delete');
		
		$lineitems_res = $this->crud->get_row_by_id('lineitems',array('module' => 4, 'parent_id' => $debit_note_id));
        if(!empty($lineitems_res)) {
            foreach ($lineitems_res as $key => $lineitem) {
                $this->crud->update_item_current_stock_qty($lineitem->item_id,$lineitem->parent_id,'debit_note',$lineitem->item_qty,'delete');       
            }
		...
		$this->crud->delete('debit_note', array('debit_note_id' => $debit_note_id));
		$this->crud->delete('lineitems', array('module' => 4, 'parent_id' => $debit_note_id));
	}
	
	/*function debit_note_detail(){
		if(!empty($_POST['debit_note_id']) && isset($_POST['debit_note_id'])){
			$result = $this->crud->get_data_row_by_id('debit_note','debit_note_id',$_POST['debit_note_id']);
			$data = array(
				'debit_note_id' => $result->debit_note_id,
				'debit_note_no' => $result->debit_note_no,
				'debit_note_date' => date('d-m-Y', strtotime($result->debit_note_date)),
				'debit_note_desc' => nl2br($result->debit_note_desc),
			...
			$where = array('module' => '4', 'parent_id' => $result->debit_note_id);
			$debit_note_lineitems = $this->crud->get_row_by_id('lineitems', $where);
			$lineitem_arr = array();
			foreach($debit_note_lineitems as $debit_note_lineitem){
				$debit_note_lineitem->item_name = $this->crud->get_id_by_val('item', 'item_name', 'item_id', $debit_note_lineitem->item_id);
				$debit_note_lineitem->item_type_name = $this->crud->get_id_by_val('item_type', 'item_type_name', 'item_type_id', $debit_note_lineitem->item_type_id);
				$debit_note_lineitem->location_name = $this->crud->get_id_by_val('location', 'location_name', 'location_id', $debit_note_lineitem->item_location_id);
				$debit_note_lineitem->rack_name = $this->crud->get_id_by_val('rack', 'rack_name', 'rack_id', $debit_note_lineitem->item_rack_id);
				$debit_note_lineitem->price_type = $this->get_price_type_label($debit_note_lineitem->price_type);
				
				$debit_note_lineitem->purchase_order_no = '';
				$debit_note_lineitem->purchase_challan_no = '';
				if(!empty($debit_note_lineitem->picker_lineitem_id)){
					$result = $this->app_model->get_module_data_by_picker_id($debit_note_lineitem->picker_lineitem_id);
					if(isset($result->purchase_order_no)){
						$debit_note_lineitem->purchase_order_no = $result->purchase_order_no;
					}
					if(isset($result->purchase_challan_no)){
						$debit_note_lineitem->purchase_challan_no = $result->purchase_challan_no;
					}
				}
				$debit_note_lineitem->before_discount_price = $debit_note_lineitem->item_qty * $debit_note_lineitem->price;
				$debit_note_lineitem->cgst_amt = $debit_note_lineitem->cgst_amount;
				$debit_note_lineitem->sgst_amt = $debit_note_lineitem->sgst_amount;
				$debit_note_lineitem->igst_amt = $debit_note_lineitem->igst_amount;
				$discount_id = array();
				$discount_type = array();
				$discount = array();
				$lineitem_discounts = $this->crud->get_row_by_id('lineitem_discount', array('lineitem_id' => $debit_note_lineitem->id));
				...
				$debit_note_lineitem->discount_id = $discount_id;
				$debit_note_lineitem->discount_type = $discount_type;
				$debit_note_lineitem->discount = $discount;
				$lineitem_arr[] = $debit_note_lineitem;
			}
