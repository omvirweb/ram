<?php
    ...
    class Sales_Invoice_from_quotation extends CI_Controller 
    {
        ...
        function purchase_quotation_add()
        {
            ...
            if($this->applib->have_access_role(MODULE_ORDER_TYPE_2_ID,"add")) {
                set_page('sales_invoice_from_quotation/purchase_quotation_add', $data);
            } else {
                $this->session->set_flashdata('success', false);
                $this->session->set_flashdata('message', 'You have not permission to access this page.');
                redirect('/');
        ...
        function purchase_quotation_list() 
        {
            if($this->applib->have_access_role(MODULE_ORDER_TYPE_2_ID,"view")) {
                ...
                set_page('sales_invoice_from_quotation/purchase_quotation_list', $data);
            } else {
                $this->session->set_flashdata('success', false);
                $this->session->set_flashdata('message', 'You have not permission to access this page.');
                redirect('/');
        ...
        function quotation_datatable()
        {
            ...
            foreach ($list as $order) {
                ...
                if($isEdit) {
                    $action .= '<form id="edit_' . $order->quotation_id . '" method="post" action="' . base_url(isset($order->quotation_type) && $order->quotation_type == '1' ? 'quotation/sales_invoice_frmquot_add' : 'quotation/purchase_quotation_add') . '" style="width: 25px; display: initial;" >
