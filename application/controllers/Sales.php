...
class Sales extends CI_Controller
{
	...
	function save_sales_invoice(){
		...
		$sales_invoice_data = array();
                ...
                if(isset($post_data['shipping_address_chkbox']) && !empty($post_data['shipping_address_chkbox'])) {
                    ...
                    $sales_invoice_data['shipping_address'] = (isset($post_data['shipping_address']) && $post_data['shipping_address'] != '') ? $post_data['shipping_address'] : '';
                }
		...
		$sales_invoice_data['pure_amount_total'] = (isset($post_data['pure_amount_total']) && $post_data['pure_amount_total'] != '') ? $post_data['pure_amount_total'] : '';
		$sales_invoice_data['discount_total'] = (isset($post_data['discounted_price_total']) && $post_data['discounted_price_total'] != '') ? $post_data['discounted_price_total'] : '';
		$sales_invoice_data['cgst_amount_total'] = (isset($post_data['cgst_amount_total']) && $post_data['cgst_amount_total'] != '') ? $post_data['cgst_amount_total'] : '';
		$sales_invoice_data['sgst_amount_total'] = (isset($post_data['sgst_amount_total']) && $post_data['sgst_amount_total'] != '') ? $post_data['sgst_amount_total'] : '';
		$sales_invoice_data['igst_amount_total'] = (isset($post_data['igst_amount_total']) && $post_data['igst_amount_total'] != '') ? $post_data['igst_amount_total'] : '';
		$sales_invoice_data['pure_amount_total'] = (isset($post_data['pure_amount_total']) && $post_data['pure_amount_total'] != '') ? $post_data['pure_amount_total'] : '';
		...
		if(isset($post_data['sales_invoice_id']) && !empty($post_data['sales_invoice_id'])){
			...
			if($result){
				...
				foreach($line_items_data[0] as $lineitem){
					...
					$add_lineitem['item_id'] = (isset($lineitem->item_id) && $lineitem->item_id != '') ? $lineitem->item_id : '';
					$add_lineitem['item_qty'] = (isset($lineitem->item_qty) && $lineitem->item_qty != '') ? $lineitem->item_qty : '';
					$add_lineitem['unit_id'] = (isset($lineitem->unit_id) && $lineitem->unit_id != '') ? $lineitem->unit_id : '';
					$add_lineitem['price'] = (isset($lineitem->price) && $lineitem->price != '') ? $lineitem->price : '';
					$add_lineitem['pure_amount'] = (isset($lineitem->pure_amount) && $lineitem->pure_amount != '') ? $lineitem->pure_amount : '';
					$add_lineitem['discount_type'] = (isset($lineitem->discount_type) && $lineitem->discount_type != '') ? $lineitem->discount_type : '';
					$add_lineitem['discount'] = (isset($lineitem->discount) && $lineitem->discount != '') ? $lineitem->discount : '';
					$add_lineitem['discounted_price'] = (isset($lineitem->discounted_price) && $lineitem->discounted_price != '') ? $lineitem->discounted_price : '';
					...
					if(isset($lineitem->id) && !empty($lineitem->id)){
                        ...
                        $sales_invoice_no = (isset($post_data['sales_invoice_no']) && $post_data['sales_invoice_no'] != '') ? $post_data['sales_invoice_no'] : '';
			...
			if($result){
                ...
                $return['is_print'] = (isset($post_data['is_print']) && $post_data['is_print'] != '') ? $post_data['is_print'] : '';
                if(isset($post_data['is_print']) && $post_data['is_print'] == 1) {
    ...
    function invoice_frm_quot_datatable(){
		/*echo "welcome";die();*/
		$from_date = '';
        $to_date = '';
        $account_id = '';
		...
		foreach ($list as $invoice) {
            ...
            $action .= '&nbsp;<a href="' . base_url('sales/format_3_invoice_print_1/' . $invoice->sales_invoice_id) . '" target="_blank" title="Formamt 3 Invoice Print" class="detail_button btn-info btn-xs"><span class="fa fa-print"></span></a>';


            if($invoice->account_group_id != CASH_IN_HAND_ACC_GROUP_ID) {
            	// $action .= '&nbsp;<a href="' . base_url('sales/format_2_invoice_print/' . $invoice->sales_invoice_id) . '" target="_blank" title="Formamt 2 Invoice Print" class="detail_button btn-info btn-xs"><span class="fa fa-print"></span></a>';
                $action .= '&nbsp;<a href="' . base_url('sales/invoice_print_new_pdf_1/' . $invoice->sales_invoice_id) . '" target="_blank" title="Tally Invoice Print" class="detail_button btn-info btn-xs"><span class="fa fa-print"></span></a>';
    ...
    function final_invoice_datatable(){
		/*echo "welcome";die();*/
		$from_date = '';
        $to_date = '';
        $account_id = '';
		...
		$config['table'] = 'final_sales_invoice si';
		$config['select'] = 'si.invoice_type, si.sales_invoice_id, si.sales_invoice_no, si.sales_invoice_date, si.pure_amount_total, a.account_name, a.account_group_id, a.account_gst_no, si.created_by, si.created_at, si.updated_by, si.updated_at, si.user_created_by, si.user_updated_by,';
		$config['column_order'] = array(null, 'si.sales_invoice_no', 'a.account_name', 'si.sales_invoice_date', 'si.pure_amount_total');
		$config['column_search'] = array('si.sales_invoice_no', 'a.account_name', 'DATE_FORMAT(si.sales_invoice_date,"%d-%m-%Y")', 'si.pure_amount_total');
		$config['wheres'][] = array('column_name' => 'si.created_by', 'column_value' => $this->logged_in_id);
		...
		foreach ($list as $invoice) {
			...
			// if($invoice->data_lock_unlock == 0){
				if($isEdit) {
					...
					if($this->is_single_line_item == 1) {
						$action .= '<form id="edit_' . $invoice->sales_invoice_id . '" method="post" action="' . base_url() . 'sales/final_sales_invoice_add/'.$invoice->sales_invoice_id.'" style="width: 25px; display: initial;" >
	                            <input type="hidden" name="sales_invoice_id" id="sales_invoice_id" value="' . $invoice->sales_invoice_id . '">
	                            <a class="edit_button btn-primary btn-xs" href="javascript:{}" onclick="document.getElementById(\'edit_' . $invoice->sales_invoice_id . '\').submit();" title="Edit Invoice"><i class="fa fa-edit"></i></a>
	                        </form> ';
					} else {
            ...
            if($invoice->account_group_id != CASH_IN_HAND_ACC_GROUP_ID) {
                ...
                if($list_type==2){
                    ...
                    $action .= '&nbsp;<a href="' . base_url('sales/final_sales_format_3_invoice_print/' . $invoice->sales_invoice_id) . '" target="_blank" title="Formamt 3 Invoice Print" class="detail_button btn-info btn-xs"><span class="fa fa-print"></span></a>';
                }
            }
...
//            echo "<pre>"; print_r($sales_invoice_lineitems); exit;
    ...
    function invoice_print_new_pdf_1($sales_invoice_id,$is_multiple = '') {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
//            echo "<pre>"; print_r($result); exit;
...
//            echo "<pre>"; print_r($sales_invoice_lineitems); exit;
        ...
        //~ echo '<pre>'; print_r($data); exit;
        $html = $this->load->view('sales/invoice/pdf_new_1', $data, true);
        $pdfFilePath = "sales_invoice.pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->AddPage('', '', '', '', '', 5, // margin_left
                5, // margin right
...
//		echo '<pre>'; print_r($data); exit;
    ...
    //new for 1
    function format_3_invoice_print_1($sales_invoice_id, $is_multiple = '') {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);            
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
            $account_detail = $this->crud->get_data_row_by_id('account', 'account_id', $result->account_id);
...
//		echo '<pre>'; print_r($data); exit;
        ...
        $data['bank_ac_no'] = isset($bank_details->bank_ac_no) ? $bank_details->bank_ac_no : '';
        $data['rtgs_ifsc_code'] = isset($bank_details->rtgs_ifsc_code) ? $bank_details->rtgs_ifsc_code : '';
        $html = $this->load->view('sales/invoice/invoice_somnath_print_1', $data, true);
    ...
    function final_sales_format_3_invoice_print($sales_invoice_id, $is_multiple = '') {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('final_sales_invoice', 'sales_invoice_id', $sales_invoice_id);            
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
            $account_detail = $this->crud->get_data_row_by_id('account', 'account_id', $result->account_id);
            $this->load->library('numbertowords');
            if ($result->pure_amount_total < 0) {
                $amount_total_word = 'Minus ' . $this->numbertowords->convert_number(abs($result->pure_amount_total));
            } else {
                $amount_total_word = $this->numbertowords->convert_number($result->pure_amount_total);
            }
            // $total_gst = $result->cgst_amount_total + $result->sgst_amount_total + $result->igst_amount_total;
            $total_gst=0;
            ...
            $data = array(
                ...
                'sales_invoice_no' => $sales_invoice_no,
                'prefix' => (isset($result->prefix) && $result->prefix != '')? $result->prefix : '',
                'sales_invoice_date' =>(isset($result->sales_invoice_date) && $result->sales_invoice_date != '') ? date('d-m-Y', strtotime($result->sales_invoice_date)) : '',
                'sales_invoice_desc' => (isset($result->sales_invoice_desc) && $result->sales_invoice_desc != '') ? nl2br($result->sales_invoice_desc) : '',
                'qty_total' => (isset($result->qty_total) && $result->qty_total != '')? $result->qty_total : '',
                'pure_amount_total' => (isset($result->pure_amount_total) && $result->pure_amount_total != '')? $result->pure_amount_total : '',
                'discount_total' => (isset($result->discount_total) && $result->discount_total != '')? $result->discount_total : '',
                'cgst_amount_total' => (isset($result->cgst_amount_total) && $result->cgst_amount_total != '')? $result->cgst_amount_total : '',
                'sgst_amount_total' => (isset($result->sgst_amount_total) && $result->sgst_amount_total != '')? $result->sgst_amount_total : '',
                'igst_amount_total' => (isset($result->igst_amount_total) && $result->igst_amount_total != '')? $result->igst_amount_total : '',
                'amount_total' => (isset($result->pure_amount_total) && $result->pure_amount_total != '')? $result->pure_amount_total : '',
                'amount_total_word' => $amount_total_word,
                'gst_total_word' => $gst_total_word,
            );
            ...
            $data['account_name'] = $account_detail->account_name;
            $cash_in_hand_acc = $this->applib->is_cash_in_hand_account($result->account_id);
            if ($cash_in_hand_acc == true && isset($result->cash_customer) && !empty($result->cash_customer)) {
                $data['account_name'] = $result->cash_customer;
            }
            $data['account_gst_no'] = $account_detail->account_gst_no;
            if (isset($result->is_shipping_same_as_billing_address) && !empty($result->is_shipping_same_as_billing_address) && $result->is_shipping_same_as_billing_address == '1') {
                $data['account_address'] = $result->shipping_address;
            } else {
                $data['account_address'] = $account_detail->account_address;
            }
            ...
            $data['account_postal_code'] = $account_detail->account_postal_code;
            $data['account_pan'] = $account_detail->account_pan;
            $data['transport_name'] = (isset($result->transport_name) && $result->transport_name != '') ? $result->transport_name : '';
            $data['lr_no'] = (isset($result->lr_no) && $result->lr_no != '') ? $result->lr_no : '';

            $lineitem_arr = array();
            $where = array('module' => '10', 'parent_id' => $sales_invoice_id);
            $sales_invoice_lineitems = $this->crud->get_row_by_id('lineitems', $where);
            $total_gst = 0;
...
//            <td width="33%" style="text-align: right; "></td></tr></table>'); // Note that the second parameter is optional : default = 'O' for ODD
    ...
    function get_invoice_no($prefix = ''){
        if($prefix != ''){
            $sales_invoice_no = $this->crud->get_max_number_where($prefix, 'sales_invoice_no', array('created_by' => $this->logged_in_id));
...
//		echo '<pre>'; print_r($data); exit;
    ...
    public function format_3_3_invoice_print($sales_invoice_id, $is_multiple = '')
    {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
            ...
            $data = array(
                ...
                'ship_party_name' => $result->ship_party_name,
                'ship_party_address' => $result->ship_party_address,
        ...
        $html = $this->load->view('sales/invoice/invoice_3_3_somnath_print', $data, true);
        $pdfFilePath = "sales_invoice_miracle_print.pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->AddPage('', '', '', '', '', 5, /* margin_left */5, /* margin right */5, /* margin top */15, /* margin bottom */5, /* margin header */5); /* margin footer */
        $this->m_pdf->pdf->SetHTMLFooter('<table width="100%" style="vertical-align: bottom; font-family: serif; font-size: 8pt; color: #000000; font-weight: bold; font-style: italic;"><tr>
		<td width="33%"><span style="font-weight: bold; font-style: italic;">{DATE j-m-Y}</span></td>
		<td width="33%" align="center" style="font-weight: bold; font-style: italic;">{PAGENO}/{nbpg}</td>
		<td width="33%" style="text-align: right; "></td></tr></table>'); // Note that the second parameter is optional : default = 'O' for ODD
    ...
    public function format_3_4_invoice_print($sales_invoice_id, $is_multiple = '')
    {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
        ...
        $html = $this->load->view('sales/invoice/invoice_3_4_somnath_print', $data, true);
        $pdfFilePath = "sales_invoice_miracle_print.pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->AddPage('', '', '', '', '', 5, /* margin_left */5, /* margin right */5, /* margin top */15, /* margin bottom */5, /* margin header */5); /* margin footer */
        $this->m_pdf->pdf->SetHTMLFooter('<table width="100%" style="vertical-align: bottom; font-family: serif; font-size: 8pt; color: #000000; font-weight: bold; font-style: italic;"><tr>
    ...
    function final_sales_invoice_add($id = '')
    {
        ...
        $data['page_title'] = 'Final Sales Invoice Add/Edit';
        ...
        if($id != ''){
            ...
            $sales_invoice_id = $id;
            $where = array('sales_invoice_id' => $sales_invoice_id);
            $sales_invoice_data = $this->crud->get_row_by_id('final_sales_invoice', $where);
            $data['sales_invoice_data'] = $sales_invoice_data[0];
            $data['sales_invoice_id'] = $sales_invoice_id;
            $lineitems = '';
            $where = array('module' => '10', 'parent_id' => $sales_invoice_id);
    ...
    function save_sales_invoice1(){
        ...
        $sales_invoice_data = [
            ...
            'sales_invoice_date'=>(isset($post_data['sales_invoice_date']) && $post_data['sales_invoice_date'] != '') ? date('Y-m-d',strtotime($post_data['sales_invoice_date'])) : date('Y-m-d'),
            'tax_type'=>(isset($post_data['tax_type']) && $post_data['tax_type'] != '') ? $post_data['tax_type'] : '',
            'our_bank_label'=>(isset($post_data['our_bank_label']) && $post_data['our_bank_label'] != '') ? $post_data['our_bank_label'] : '',
            'sales_invoice_no'=>(isset($post_data['sales_invoice_no']) && $post_data['sales_invoice_no'] != '') ? $post_data['sales_invoice_no'] : '',
            'site_id'=>(isset($post_data['site_id']) && $post_data['site_id'] != '') ? $post_data['site_id'] : '',
            'sales_rate_type'=>(isset($post_data['sales_rate_type']) && $post_data['sales_rate_type'] != '') ? $post_data['sales_rate_type'] : '',
            'qty_total'=>(isset($post_data['qty_total']) && $post_data['qty_total'] != '') ? $post_data['qty_total'] : '',
            'pure_amount_total'=>(isset($post_data['pure_amount_total']) && $post_data['pure_amount_total'] != '') ? $post_data['pure_amount_total'] : '',
            'discounted_price_total'=>(isset($post_data['discounted_price_total']) && $post_data['discounted_price_total'] != '') ? $post_data['discounted_price_total'] : '',
            'round_off_amount'=>(isset($post_data['round_off_amount']) && $post_data['round_off_amount'] != '') ? $post_data['round_off_amount'] : '',
            'sales_invoice_desc'=>(isset($post_data['sales_invoice_desc']) && $post_data['sales_invoice_desc'] != '') ? $post_data['sales_invoice_desc'] : '',
            'invoice_type' => isset($post_data['invoice_type']) ? $post_data['invoice_type'] : null,
        ...
        if(isset($post_data['sales_invoice_id']) && !empty($post_data['sales_invoice_id'])){
			...
			$sales_invoice_result = $this->crud->get_row_by_id($post_data['table'], $where);
			if(!empty($sales_invoice_result) && $sales_invoice_result != $post_data['sales_invoice_id']){
				echo json_encode(array("error" => 'Exist'));
				exit;
			}
            ...
            $sales_invoice_data['user_updated_by'] = $this->session->userdata()['login_user_id'];
            $where_array['sales_invoice_id'] = $post_data['sales_invoice_id'];
            $result = $this->crud->update($post_data['table'], $sales_invoice_data, $where_array);
            if($result){
				$return['success'] = "Updated";
				$this->session->set_flashdata('success',true);
				$this->session->set_flashdata('message','Sales Invoice Updated Successfully');
				...
				foreach($line_items_data[0] as $lineitem){
					...
					$add_lineitem['other_charges'] = (isset($lineitem->other_charges) && $lineitem->other_charges != '') ? $lineitem->other_charges : '';
					$add_lineitem['amount'] = (isset($lineitem->amount) && $lineitem->amount != '') ? $lineitem->amount : '';
					$add_lineitem['module'] = $post_data['module'];
					$add_lineitem['parent_id'] = $parent_id;
                    $add_lineitem['l'] = (isset($lineitem->l) && $lineitem->l != '') ? $lineitem->l : '';
                    $add_lineitem['b'] = (isset($lineitem->b) && $lineitem->b != '') ? $lineitem->b : '';
                    $add_lineitem['d'] = (isset($lineitem->d) && $lineitem->d != '') ? $lineitem->d : '';
			...
			$where = array('sales_invoice_no' => $sales_invoice_no, 'created_by' => $this->logged_in_id);
            ...
            $result = $this->crud->insert($post_data['table'],$sales_invoice_data);
            if($result){
                $return['success'] = "Added";
                $this->session->set_flashdata('success',true);
                $this->session->set_flashdata('message','Sales Invoice Added Successfully');
