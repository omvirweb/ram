...
//            echo '<pre>'; print_r($lineitem_arr); exit;
    ...
    function format_3_invoice_print($sales_invoice_id, $is_multiple = '' ,$new=false) {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            ...
            $data['account_gst_no'] = $account_detail->account_gst_no;
            if (!empty($result->is_shipping_same_as_billing_address) && $result->is_shipping_same_as_billing_address == '1') {
                $data['account_address'] = $account_detail->account_address;//actually $result->shipping_address;
            } else {
                $data['account_address'] = $account_detail->account_address;
            }
            $data['account_country'] = 'India';
...
//		echo '<pre>'; print_r($data); exit;
        ...
        $pdfFilePath = "sales_invoice_miracle_print_".$result->sales_invoice_id."_".time().".pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->AddPage('', '', '', '', '', 5, // margin_left
                5, // margin right
                5, // margin top
    ...
    function format_3_invoice_print_sign($sales_invoice_id, $is_multiple = '' ,$new=false) {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);            
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
            $account_detail = $this->crud->get_data_row_by_id('account', 'account_id', $result->account_id);
...
//		echo '<pre>'; print_r($data); exit;
        ...
        $data['bank_ac_no'] = isset($bank_details->bank_ac_no) ? $bank_details->bank_ac_no : '';
        $data['rtgs_ifsc_code'] = isset($bank_details->rtgs_ifsc_code) ? $bank_details->rtgs_ifsc_code : '';
        $html = $this->load->view('sales/invoice/invoice_somnath_print_with_stamp', $data, true);
    ...
    function format_3_invoice_print_n($sales_invoice_id, $is_multiple = '') {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);            
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
            $account_detail = $this->crud->get_data_row_by_id('account', 'account_id', $result->account_id);
...
//		echo '<pre>'; print_r($data); exit;
        ...
        $data['bank_ac_no'] = isset($bank_details->bank_ac_no) ? $bank_details->bank_ac_no : '';
        $data['rtgs_ifsc_code'] = isset($bank_details->rtgs_ifsc_code) ? $bank_details->rtgs_ifsc_code : '';
        $html = $this->load->view('sales/invoice/invoice_somnath_print_n', $data, true);
...
//		echo '<pre>'; print_r($data); exit;
    ...
    function format_3_2_invoice_print_sign($sales_invoice_id, $is_multiple = '') {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
            $account_detail = $this->crud->get_data_row_by_id('account', 'account_id', $result->account_id);
...
//		echo '<pre>'; print_r($data); exit;
        ...
        $html = $this->load->view('sales/invoice/invoice_3_2_somnath_print_stamp', $data, true);
        /*echo $html;exit;*/
        $pdfFilePath = "sales_invoice_miracle_print.pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->AddPage('', '', '', '', '', 5, // margin_left
    ...
    function format_3_2_invoice_print_stax_before($sales_invoice_id, $is_multiple = '') {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
            $account_detail = $this->crud->get_data_row_by_id('account', 'account_id', $result->account_id);
...
//		echo '<pre>'; print_r($data); exit;
    ...
    public function format_3_4_invoice_print_sign($sales_invoice_id, $is_multiple = '')
    {
        if (!empty($sales_invoice_id) && isset($sales_invoice_id)) {
            $result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);
            $user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
        ...
        $html = $this->load->view('sales/invoice/invoice_3_4_somnath_print_stamp', $data, true);
        $pdfFilePath = "sales_invoice_miracle_print.pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->AddPage('', '', '', '', '', 5, /* margin_left */5, /* margin right */5, /* margin top */15, /* margin bottom */5, /* margin header */5); /* margin footer */
        $this->m_pdf->pdf->SetHTMLFooter('<table width="100%" style="vertical-align: bottom; font-family: serif; font-size: 8pt; color: #000000; font-weight: bold; font-style: italic;"><tr>
    ...
    //         'discount_total'=>(isset($post_data['discounted_price_total']) && $post_data['discounted_price_total'] != '') ? $post_data['discounted_price_total'] : '',
    ...
    public function download_multiple_invoices()
    {
    	...
    	$termsdata = $this->crud->get_column_value_by_id('settings', 'setting_value', array('setting_key' => 'sales_terms'));
    	$invoice_data = [];
    	$invoice_ids = explode(',', $post_data['invoice_ids']);

    	foreach($invoice_ids as $sales_invoice_id) {
    		$result = $this->crud->get_data_row_by_id('sales_invoice', 'sales_invoice_id', $sales_invoice_id);            
    		$user_detail = $this->crud->get_data_row_by_id('user', 'user_id', $result->created_by);
    		$account_detail = $this->crud->get_data_row_by_id('account', 'account_id', $result->account_id);
    		$this->load->library('numbertowords');
    	...
    	$html = $this->load->view('sales/invoice/invoice_somnath_print_multiple', ['invoice_data' => $invoice_data], true);

    	$pdfFilePath = "sales_invoice_miracle_print_multiple_".time().".pdf";
